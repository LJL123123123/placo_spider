...
def main():
    ...
    leg_swim_task = {
        ...
        "LF": solver.add_relative_position_task("body","leg2", np.array([0.02, -0.15, 0.0])),
        "RF": solver.add_relative_position_task("body","leg3", np.array([0.15, 0.0, 0.0])),
        "RH": solver.add_relative_position_task("body","leg4", np.array([0.02, 0.15, 0.0])),
    }
    # 配置足端任务优先级
    ...
    # body task
    body_task = solver.add_position_task("body", np.array([0.0, 0.0, 0.25]))
    body_init_task = solver.add_frame_task("body", tf.translation_matrix([0.0, 0.0, 0.25]))
    ...
    def loop():
        ...
        body_pos = np.array([0.0 + move_length * (t / duration), 0.0, 0.25])
        ...
        for foot in cpg.foot_names:
            ...
            if foot == "LF":
                foot_pos["LF"] = cpg.generate_foot_position(foot, t) + np.array([0.0, 0.0, 0.50])
            elif foot == "LH":
                foot_pos["LH"] = cpg.generate_foot_position(foot, t) + np.array([0.0, 0.0, 0.50])
            elif foot == "RF":
                foot_pos["RF"] = cpg.generate_foot_position(foot, t) + np.array([0, 0, 0.50])
            elif foot == "RH":
                foot_pos["RH"] = cpg.generate_foot_position(foot, t) + np.array([0.0, 0.0, 0.50])
